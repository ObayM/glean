...
describe('buildDuplicateQuery', () => {
  ...
  it('escapes quotes and backslashes in both the deck name and the word', () => {
    expect(buildDuplicateQuery('My "Deck"', 'back\\slash', 'Word')).toBe(
      'deck:"My \\"Deck\\"" ("Word:back\\\\slash" or "Word:Back\\\\slash")'
    );
  });

  it('uses the given field name instead of the hardcoded "Word"', () => {
    expect(buildDuplicateQuery('Glean', 'cat', 'Front')).toBe(
      'deck:"Glean" ("Front:cat" or "Front:Cat")'
    );
  });
});
